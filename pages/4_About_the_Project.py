...
# --- 1. THE WHY & THE WHAT ---
# High-level summary of the project goals.
st.markdown("""
...
# Force 2.1: Gravity
# Explains the sine component of gravity (acting along the slope).
st.markdown("#### **2.1. Gravity Force ($F_{grav}$)**") # Diagram trigger added here to visualize vectors ***
...
# Force 2.2: Rolling Resistance
# Explains the cosine component (Normal Force) affecting friction.
...
# Force 2.3: Aerodynamic Drag
# Highlight the v^2 relationship (exponential increase).
...
# --- 5. SIMULATION LOOP ---
# Describes the numerical integration method (Newtonian Mechanics).
...
# --- 6. GENETIC ALGORITHM ---
# Explains the evolutionary search logic in simple terms.
...
""") # Diagram trigger added here to explain the process flow
...
st.markdown("#### **The 'Battery' Analogy**") # Diagram trigger added here to visualize CP vs W' ***
st.markdown("""
...
st.markdown("""
The engine uses this model to determine **Maximum Sustainable Power**. It understands that a rider's capability changes based on the duration of the effort:
...
# --- 8. NUTRITION SECTION ---
st.subheader("7. Nutrition & Metabolic Modelling")
st.markdown("""
To provide realistic fueling advice, the model tracks energy expenditure using three physiological principles: 
**Gross Efficiency**, **Fuel Source**, and **Absorption Limits**.
""")

st.markdown("#### **7.1. Energy Expenditure**")
...
* $\eta_{gross}$ – Gross Efficiency (~24%)

**The 'Engineer's Shortcut':**
While 1 kcal technically equals 4.184 kJ, the human body is roughly 24% efficient on a bike. This creates a convenient cancellation: **1 kJ of output on the power meter is roughly equal to 1 kcal of energy burned.**
""")

st.markdown("#### **7.2. Fuel Source (Carbs vs. Fat)**")
st.markdown("Not all calories are created equal. The engine estimates the ratio of Carbohydrates (Glycogen) to Fat being burned based on your **Intensity Factor (IF)**.")
st.markdown("") # Trigger diagram for Carb/Fat oxidation *** # Diagram trigger added here to visualize the fuel switch
st.markdown("""
* **Low Intensity (IF < 0.6):** You are burning a mix of fat and carbs. We conservatively assume **40% Carbs** to ensure you don't under-fuel.
* **High Intensity (IF > 1.0):** Above Threshold, your body relies almost exclusively on fast-burning fuel. We assume **100% Carbs**.
* **The 'Grey Zone':** Between these points, the model linearly interpolates the ratio to guess your specific needs.
""")

st.markdown("#### **7.3. Required Carb Intake**")

st.markdown("""
The "Required Carb Intake" value isn't a random guess. It is calculated using a step function based on ride duration, following standard endurance guidelines:
""")

# Equation with "Where" clause formatting
st.latex(r"Total_{Carbs} = Rate_{hourly} \times Duration_{hours}")

st.markdown("""
Where:
* $Total_{Carbs}$ – Total recommended intake (g)
* $Rate_{hourly}$ – Intake target based on intensity step (g/hr)
* $Duration_{hours}$ – Total ride time (hrs)

This ensures the advice scales linearly with your ride time, but jumps in intensity as you cross critical physiological thresholds.

**The Hourly Targets ($Rate_{hourly}$):**
* **< 1h 15m:** 0g/hr (Rely on stored glycogen)
* **1h 15m – 2h 30m:** 45g/hr (Small top-up)
* **2h 30m – 4h 00m:** 75g/hr (Standard fueling)
* **> 4h 00m:** 90g/hr (Maximal absorption)

Replacing calories 1-for-1 is impossible, as the body's burn rate is much higher than its maximum absorption rate. Full replenishment is physiologically impossible during intense exercise.
* **Burn Rate:** A fit rider can easily burn **800–1,000 kcal/hr**.
* **Absorption Rate:** The human gut acts as a bottleneck, typically processing only **240–360 kcal/hr** (60–90g of carbs).

**The Deficit Strategy:**
The model calculates recommendations based on your Absorption Limits, not just your Burn Rate. 
It acknowledges that the massive energy deficit must be bridged by the glycogen already stored in your liver and muscles (~400-500g).
""")

# --- 9. Model Limitations ---
st.markdown("---")
st.subheader("8. Model Limitations")
st.markdown("""
**Accuracy vs. Reality:**
While the physics engine uses industry-standard equations (Newtonian mechanics, Critical Power model), the output is an **estimate**. 
Real-world accuracy depends heavily on:
* **Input Precision:** A 0.002 error in Crr (Tyres) can change a finish time significantly.
* **Environmental Variance:** Wind speed is rarely constant across a whole route.
* **Biomechanical Efficiency:** Rider position and efficiency fluctuate during fatigue.
* **Human Variability (Stochastic Power):** The physics engine applies force consistently every second. A human rider micro-accelerates and decelerates. 
  Because aerodynamic drag is non-linear ($v^2$), a variable power output is generally *slower* than a perfectly constant one for the same average watts. 
  Therefore, the model represents the "Maximum Theoretical Efficiency.
""")

st.markdown("---")

# --- 10. ARCHITECTURE ---
with st.expander("🛠️ System Architecture & Data Processing (Click to Expand)"):
