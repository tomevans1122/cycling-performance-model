...
# W' Input
w_prime_kj = st.sidebar.number_input("Anaerobic Capacity (W' in kJ)", value=14.0, step=0.5, min_value=1.0, help="Energy battery above FTP. Avg cyclist = 14kJ, Pro = 25kJ+")
w_prime_j = w_prime_kj * 1000.0

# 2. AERODYNAMICS
st.sidebar.header("2. Aerodynamics (CdA)")
st.sidebar.info("Model automatically adjusts for climbing/descending positions.")
cda_presets = {
...
if file_path_to_process:
    try:
        ...
        # 3. CALCULATION
        st.info("ℹ️ **Processing Note:** This tool must run the physics engine multiple times (iteratively) to find the exact match. Please allow **~3 minutes** for the calculation to complete.")
        ...
        if st.button("Calculate Required Watts"):
            ...
            with st.spinner("Iterating physics engine... (This may take ~3 minutes)"):
